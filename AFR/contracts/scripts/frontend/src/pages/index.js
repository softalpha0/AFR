...
export default function Home() {
  ...
  // ---- HELPERS ----
  const displayUsdc = (val) =>
    val ? Number(formatUnits(val, MUSDC_DECIMALS)).toLocaleString() : "0";

  const displayShares = (val) =>
    val ? Number(formatUnits(val, 18)).toLocaleString() : "0";
  ...
  const handleDeposit = async () => {
    ...
    console.log("MUSDC_ADDRESS at runtime:", MUSDC_ADDRESS);
    console.log("VAULT_ADDRESS at runtime:", VAULT_ADDRESS);

    try {
      const amount = parseUnits(depositAmount, MUSDC_DECIMALS);

      // 1. approve vault to spend mUSDC
  ...
  return (
    ...
    >
      ...
      >
        <h1 style={{ fontSize: "1.8rem", fontWeight: 600 }}>AFR Savings Vault</h1>
        <ConnectButton />
      </div>
      ...
      >
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: "1.1rem", fontWeight: 600 }}>
                    {displayUsdc(mUsdcBalance)} mUSDC
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: "0.8rem", opacity: 0.7 }}>
                  ...
                  <div style={{ fontSize: "1.1rem", fontWeight: 600 }}>
                    {displayShares(shareBalance)} aSHARE
                  </div>
                </div>
              </div>
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            <StatRow
              label="Total assets"
              value={`${displayUsdc(totalAssets)} mUSDC`}
            />
            <StatRow
              label="Total shares"
              value={`${displayShares(totalShares)} aSHARE`}
            />
            <StatRow
              label="Implied share price"
              value={`${sharePrice.toFixed(4)} mUSDC per share`}
            />
          </div>
        </div>
      </div>
